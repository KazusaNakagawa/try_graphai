...
program.option(
  ...
  '../config/test_ai_stock_analysis.yaml',
)
  ...
  nodes: {
    company_data: {
      ...
      params: {
        model: model,
        system:
          'あなたはAI技術企業を専門とする金融アナリストです。以下の観点から詳細な分析を日本語でMarkdown形式で提供してください:',
      },
      inputs: {
    ...
    investment_recommendation: {
      ...
      params: {
        model: model,
        system:
          'すべての分析データに基づいて、詳細な投資推奨を日本語でMarkdown形式で提供してください。リスクと機会の両面から説明してください:',
      },
      inputs: {
...
const main = async () => {
  ...
  for (const company of config.companies) {
    const outputPath = path.join(
      outputDir,
      `analysis_${company.name.toLowerCase()}_${timestamp}.md`,
    )
